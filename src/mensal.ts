...
function apagarVendasDoMes(): void {
    const selectMes = document.getElementById("mes") as HTMLSelectElement;
    const mesSelecionado = selectMes.value;
    ...
    const historicoAtualizado = historicoCompleto.filter(pedido => {
        if (!pedido.data) return true; // Mantém pedidos sem data (para segurança)
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    selectMes?.addEventListener("change", carregarVendasMensais);
    
    // Evento do botão apagar
    btnApagar?.addEventListener("click", apagarVendasDoMes);
